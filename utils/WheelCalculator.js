...
export class WheelCalculator {
   ...
   * @param {number} currentRotation - 當前輪盤位置（可選，默認0）
   * @returns {Object} 包含總旋轉角度的對象
   */
  static calculateSpin(options, currentRotation = 0) {
    ...
    // 3. 生成安全的最終角度（避免邊界問題）
    const randomFinalAngle = this.generateSafeFinalAngle(segmentAngle);
  ...
  /**
   * 生成安全的最終角度（避免停在扇形邊界）
   * @param {number} segmentAngle - 每個扇形的角度
   * @returns {number} 安全的最終角度
   */
  static generateSafeFinalAngle(segmentAngle) {
  ...
  /**
   * 根據最終角度計算結果（0度指針位置決定）
   * @param {number} finalRotation - 最終旋轉角度
   * @param {Array} options - 選項數組
   * @returns {Object} 計算結果
   */
  static calculateResult(finalRotation, options) {
    const segmentAngle = 360 / options.length;
  ...
  /**
   * 獲取調試信息
   * @param {Object} spinData - 旋轉數據
   * @param {Object} result - 結果數據
   * @param {Array} options - 選項數組
   * @returns {Object} 調試信息
   */
  static getDebugInfo(spinData, result, options) {
    return {
      旋轉參數: {
        ...
        相對旋轉角度: Math.round(spinData.relativeRotation * 100) / 100 + "度",
